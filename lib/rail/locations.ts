...
export function resolveLocation(input: string): RailLocation {
  ...
  const partial = locations.find(x =>
    x.name.toLowerCase().includes(value) ||
    x.stations.some(s => s.toLowerCase().includes(value))
  );
  return partial ?? { name: input, type: 'location', stations: [input] };
}

export function searchLocations(input: string): RailLocation[] {
  const value = input.trim().toLowerCase();
  if (!value) return [];
  return locations
    .filter(x =>
